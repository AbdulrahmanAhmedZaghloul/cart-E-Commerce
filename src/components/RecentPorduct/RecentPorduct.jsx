...
export default function RecentProduct() {
    ...
    return (
        <React.Fragment>
            <div id="scrollTop" className='container mx-auto py-10 my-4 px-4'>
               
                <div   className='flex justify-center mx-auto items-center my-5'>
                    {currentPage > 1 && (
                        <span onClick={() => handlePageChange(currentPage - 1)} className='text-gray-500 mx-2'>
                            <i className="fa-solid fa-angles-left"></i>
                        </span>
                    )}
                    {[...Array(totalPages).keys()].map(pageNumber => (
                        <span key={pageNumber} onClick={() => handlePageChange(pageNumber + 1)} className={`px-4 py-2 mx-2 rounded bg-gray-100 ${currentPage === pageNumber + 1 ? 'text-gray-600 font-semibold' : 'text-gray-500'}`}>
                            {pageNumber + 1}
                        </span>
                    ))}
